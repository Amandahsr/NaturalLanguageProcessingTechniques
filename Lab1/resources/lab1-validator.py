...
def validate_line(linenum, line, sourceline):
    if re.search(r"[^0-9 ]", line):
        fail(
            linenum,
            "File contains characters other than digits, spaces, and newlines",
        )
    if not re.search(r"\d", line):
        fail(
            linenum, "File contains blank line(s)"
        )
    nums = list(map(int, line.split()))
    num_declared_offsets = nums.pop(0)
    num_provided_offsets = len(nums)
    if (
        num_declared_offsets
        != num_provided_offsets
    ):
        fail(
            linenum,
            f"Number of offsets declared ({num_declared_offsets}) does not match number of offsets provided ({num_provided_offsets})",
        )
    for index in range(num_provided_offsets - 1):
        if nums[index] >= nums[index + 1]:
            fail(
                linenum,
                "Offsets are not in increasing numerical order",
            )
    if sourceline:
        if nums[-1] >= len(sourceline):
            fail(
                linenum, "Offset past end of line"
            )
...
def validate(filename, sourcefile):
    ...
    if sourcefile:
        ...
        if len(sourcelines) != len(lines):
            fail(
                -1,
                f"Number of lines in submission file ({len(lines)}) does not match number of lines in source file ({len(sourcelines)})",
            )
...
def main():
    ...
    parser.add_argument(
        "submissionfile",
        help="File containing output of your Lab 1 Part D (sentence boundary detection) program",
    )
    parser.add_argument(
        "sourcefile",
        nargs="?",
        help="Optional source file of lines that were split into sentences",
    )

    args = vars(parser.parse_args())
    validate(
        args["submissionfile"], args["sourcefile"]
    )
